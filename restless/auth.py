    ...
    def authenticate(self, request):
        if 'authorization' in request.headers:
            ...
            if len(authdata) == 2 and authdata[0].lower() == "basic":
                ...
                if user is not None and user.is_active:
                    # We don't user auth.login(request, user) because may be running without session
                    request.user = user
...
class AuthenticateEndpoint(Endpoint, UsernamePasswordAuthMixin):
    """
    Session-based authentication API endpoint. Provides a GET method for authenticating the user based on passed-in
    "username" and "password" request params. On successful authentication, the method returns authenticated user
    details.
